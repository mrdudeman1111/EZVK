...
// Allocated Image
    ...
    {
        Image->AttachmentDesc.format = Image->Format;
        Image->AttachmentDesc.samples = VK_SAMPLE_COUNT_1_BIT;
        Image->AttachmentDesc.loadOp = LoadOp;
        Image->AttachmentDesc.storeOp = StoreOp;
        Image->AttachmentDesc.stencilLoadOp = StencilLoadOp;
        Image->AttachmentDesc.stencilStoreOp = StencilStoreOp;
        Image->AttachmentDesc.initialLayout = InitLayout;
        Image->AttachmentDesc.finalLayout = FinalLayout;

        Image->AttachmentDesc.Layout = Image->Layout;
    }

// Frame Buffer
    void Image::BuildFrameBufferImages(VkExtent3D ImageExtent, VkFormat ImageFormat, FrameBuffer* Framebuffer, VkImageViewType ViewType)
    {
        // These Images are already allocated and instantiated, they are created with the Swapchain
        Image::AllocateImageView(ViewType, VK_IMAGE_ASPECT_COLOR_BIT, &Framebuffer->ImageBuffer);
        Image::BuildRenderTarget(VK_IMAGE_LAYOUT_UNDEFINED, VK_IMAGE_LAYOUT_PRESENT_SRC_KHR, &Framebuffer->ImageBuffer, VK_ATTACHMENT_LOAD_OP_CLEAR, VK_ATTACHMENT_STORE_OP_STORE);
        ...
        Image::BuildRenderTarget(VK_IMAGE_LAYOUT_UNDEFINED, VK_IMAGE_LAYOUT_DEPTH_STENCIL_ATTACHMENT_OPTIMAL, &Framebuffer->DepthBuffer, VK_ATTACHMENT_LOAD_OP_CLEAR, VK_ATTACHMENT_STORE_OP_STORE, VK_ATTACHMENT_LOAD_OP_CLEAR, VK_ATTACHMENT_STORE_OP_DONT_CARE);
