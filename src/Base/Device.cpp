...
namespace Ek
{
    ...
    AllocatedImage Device::CreateTexture(VkImageType ImageType, VkExtent3D Extent, VkFormat ImageFormat, VkImageLayout Layout, VmaMemoryUsage MemUse, uint32_t MipLevels, VkImageUsageFlags ImageUsage)
    {
    ...
    AllocatedImage Device::LoadTexture(const char* Path, VkExtent3D* Extent)
    {
        sail::image Image(Path);

        // Figure out if we need to change the format
        Image.convert(SailPixelFormat::SAIL_PIXEL_FORMAT_BPP32_RGBA);
        AllocatedImage Texture = CreateTexture(VK_IMAGE_TYPE_3D, *Extent, VK_FORMAT_R32G32B32A32_SFLOAT, VK_IMAGE_LAYOUT_GENERAL, VmaMemoryUsage::VMA_MEMORY_USAGE_CPU_TO_GPU, 1, VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT);
        void* DstMem;

        vmaMapMemory(Allocator, Texture.Allocation, &DstMem);
        memcpy(DstMem, Image.pixels(), sizeof(Image.pixels()));
    ...
    void Device::Create(Instance* Inst, PhysicalDevice* PDevice, QueueList DesiredQueues, Ek::Queues* Queues)
    {
        ...
        int GraphicsCount = (DesiredQueues[QueueType::Graphics] > FamilyIndices.GraphicsCount) ? FamilyIndices.GraphicsCount : DesiredQueues[QueueType::Graphics];
        int ComputeCount = (DesiredQueues[QueueType::Compute] > FamilyIndices.ComputeCount) ? FamilyIndices.ComputeCount : DesiredQueues[QueueType::Compute];
        int TransferCount = (DesiredQueues[QueueType::Transfer] > FamilyIndices.TransferCount) ? FamilyIndices.TransferCount : DesiredQueues[QueueType::Transfer];
        int SparseBindCount = (DesiredQueues[QueueType::SparseBind] > FamilyIndices.SparseBindingCount) ? FamilyIndices.SparseBindingCount : DesiredQueues[QueueType::SparseBind];
        int ProtectedCount = (DesiredQueues[QueueType::Protected] > FamilyIndices.ProtectedCount) ? FamilyIndices.ProtectedCount : DesiredQueues[QueueType::SparseBind];
        int OpticFlowCount = (DesiredQueues[QueueType::OpticFlow] > FamilyIndices.OpticFlowCount) ? FamilyIndices.OpticFlowCount : DesiredQueues[QueueType::OpticFlow];

        #ifdef VK_ENABLE_BETA_EXTENSIONS
            int DecodeCount = (DesiredQueues[QueueType::Decode] > FamilyIndices.DecodeCount) ? FamilyIndices.DecodeCount : DesiredQueues[QueueType::Decode];
            int EncodeCount = (DesiredQueues[QueueType::Encode] > FamilyIndices.EncodeCount) ? FamilyIndices.EncodeCount : DesiredQueues[QueueType::Encode];
        #endif
